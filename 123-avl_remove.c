...
/**
 * avl_remove - a function that removes a
 * node from an AVL tree
 *
 * @root: apointer to the root node of the tree
 * for removing a node
 *
 ...
 * equals to value must be removed and freed
 * If the node to be deleted has two children,
 * it must be replaced with its first
...
{
	int b_factor;
	avl_t *tmp = root, *node = NULL, *ptr = NULL;

	if (root == NULL)
		return (NULL);
	tmp = bst_remove(root, value);


	node = tree_balancer(tmp);
